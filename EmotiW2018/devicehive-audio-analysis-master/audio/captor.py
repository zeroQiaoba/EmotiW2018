...
logger = logging.getLogger('audio_analysis.captor') # logger is like print
...
class Captor(object):
    ...
    def _capture(self):
        ...
        while not self._shutdown_event.is_set():
            ...
            buf = ad.read(self._sample_rate) # gain 1s audio
            if buf is None:
                logger.debug('Buffer is empty.')
                return
            capture_buf += buf # save in capture_buf

            overflow = len(capture_buf) - self._max_data
            if overflow > 0:
                logger.info('Buffer overflow, truncate {}b.'.format(overflow))
                capture_buf = capture_buf[overflow:] # only gain last _max_data if overflow
