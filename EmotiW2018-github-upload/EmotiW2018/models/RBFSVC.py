...
class RBFSVC(BasicModule):
	def __init__(self, opt):
		super(RBFSVC, self).__init__()
		self.model_name = 'RBFSVC'
		self.opt=opt
	...
	def train(self, X_train, y_train, X_val, y_val):
		...
		for epoch in range(self.opt.max_epoch):
			for C_value in range(-3,16,1):
				linear_svm = SVC(C = pow(2, C_value), probability = True, kernel = 'rbf')
