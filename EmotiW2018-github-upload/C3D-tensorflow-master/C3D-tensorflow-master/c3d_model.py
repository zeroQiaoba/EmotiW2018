...
def inference_c3d(_X, _dropout, batch_size, _weights, _biases):
  ...
  dense1_1 = tf.matmul(dense1, _weights['wd1']) + _biases['bd1'] # 8192->4096
  dense1_2 = tf.nn.relu(dense1_1, name='fc1') # Relu activation
  dense1_3 = tf.nn.dropout(dense1_2, _dropout)

  dense2_1 = tf.matmul(dense1_3, _weights['wd2']) + _biases['bd2'] # 4096->4096
  dense2_2 = tf.nn.relu(dense2_1, name='fc2') # Relu activation
  dense2_3 = tf.nn.dropout(dense2_2, _dropout)

  # Output: class prediction
  out = tf.matmul(dense2_3, _weights['out']) + _biases['out']

  return dense1_1, dense2_1, out
